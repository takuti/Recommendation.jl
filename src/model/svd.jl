...
Recommendation based on SVD of a user-item matrix ``R \\in \\mathbb{R}^{|\\mathcal{U}| \\times |\\mathcal{I}|}``, which was originally studied by [Sarwar et al.](http://files.grouplens.org/papers/webKDD00.pdf) Rank ``k`` is configured by `n_factor`. Sparse matrix is not supported for `data.R`.
...
struct SVD <: Recommender
    ...
    function SVD(data::DataAccessor, n_factor::Integer)
        n_user, n_item = size(data.R)
        U = matrix(n_user, n_factor)
        S = vector(n_factor)
        Vt = matrix(n_factor, n_item)
        new(data, n_factor, U, S, Vt)
    end
end
...
function fit!(recommender::SVD)
    ...
    recommender.U[:] = res.U[:, 1:recommender.n_factor]
    recommender.S[:] = res.S[1:recommender.n_factor]
    recommender.Vt[:] = res.Vt[1:recommender.n_factor, :]
end
